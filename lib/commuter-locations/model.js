...
/**
 * Find CommuterLocations for a single location. Includes option for paginated retrieval.
 */

schema.statics.findByLocation = function (_location, offset, limit) {
...
/**
 * Find CommuterLocations for an array of one or more location IDs.
 */

schema.statics.findByLocations = function (locations) {
  return new Promise((resolve, reject) => {
    this.find({ '_location': { $in: locations } })
      .populate('_commuter _location')
      .exec()
      .then(resolve)
      .catch(reject)
...
/**
 * Find CommuterLocations for a commuter.
 */

schema.statics.findByCommuter = function (_commuter) {
  return new Promise((resolve, reject) => {
    this.find()
...
/**
 * Get number of CommuterLocations for a location.
 */
...
schema.statics.notifyManagers = function (_location, stormpathClient) {
  ...
  Location.findById(_location, (err, location) => {
    ...
    this.findByLocation(_location).then((cls) => {

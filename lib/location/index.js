...
module.exports = createRouter({
  ...
  router.delete('/:id', stormpath.authenticationRequired, stormpath.groupsRequired([group.administrator()]), byId, function (req, res) {
    // find any associated CommuterLocations
    CommuterLocation.findByLocation(req.location).then(cls => {
      // set up promises to delete each associated CommuterLocation
      var jobs = cls.map(cl => {
        return new Promise(function (resolve, reject) {
          cl.remove(function (err) {
            if (err) {
              reject(err)
            } else {
              resolve()
      ...
      // add promise to delete the Location itself
